...
import { buildCivitaiMetadataAdvice } from "@/lib/civitai-metadata-advice";
...
function parseImportParams(meta: Record<string, unknown>, item: CivitaiImageItem) {
  const version = stringValue(meta.Version ?? meta.version);
  const modelName = stringValue(meta.Model ?? meta.model ?? meta.ModelName);
  const isComfyUiMetadata = /comfyui/i.test(version);
  // An explicit source backend is stronger evidence than a model-name heuristic.
  // This A-mix image was generated by ComfyUI at its full recorded resolution.
  const needsForgeCompatibility =
    !isComfyUiMetadata && /a.?mix.*illustrious/i.test(modelName);
  const usesA1111 = Boolean(
    needsForgeCompatibility || (version && !/comfyui/i.test(version))
  );
  const params: Partial<GenerationParams> = {
    backend: needsForgeCompatibility ? "forge" : usesA1111 ? "a1111" : "comfyui",
  ...
  else if (inferredHires && needsForgeCompatibility) params.hires_steps = 10;
  ...
  } else if (inferredHires && needsForgeCompatibility) {
    // Preserve Forge's native latent Hires path when the final dimensions imply
    // a second pass but Civitai omitted the upscaler field.
    params.upscale_model_name = "Latent";
  }
  if (!denoise && inferredHires && needsForgeCompatibility) {
    params.denoise_strength = 0.1;
  }
