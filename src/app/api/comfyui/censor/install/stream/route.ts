...
// Streams `setup:comfyui-censor` (censor custom nodes + NudeNet detector) for the
...
export async function POST() {
  ...
  const script = join(
    process.cwd(),
    "scripts",
    windows ? "setup-comfyui-censor.ps1" : "setup-comfyui-censor.sh"
  );
  if (!existsSync(script)) {
    return Response.json(
      { error: `Censor setup script not found at ${script}.` },
      { status: 500 }
    );
  }
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      safeSend({ type: "status", message: "Starting censor install (nodes + detector)..." });
      ...
      child.on("close", (code) => {
        ...
        if (code === 0) {
          safeSend({
            type: "complete",
            message:
              "검열 설치 완료. ComfyUI를 재시작한 뒤 다시 생성하세요. (Censor installed — restart ComfyUI, then generate again.)",
          });
        } else {
          safeSend({
            type: "error",
            message: `Censor install exited with code ${code}. See logs above.`,
          });
        }
        if (!closed) {
          closed = true;
