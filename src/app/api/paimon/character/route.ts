...
const PAIMON_SYSTEM_PROMPT = [
  ...
  "- backgroundDescription:string — natural-language description of the character's typical background/setting.",
  "- backgroundPrompt:string — the background rewritten as a generation prompt (environment, location, time of day, atmosphere). No subject/person tags.",
  "- situations:array — each item { id?:string, name:string, description:string, prompt:string }. Each is a scene/action the character can be in (e.g. floating peacefully in the sea). description is natural language, prompt is generation-ready (pose, action, camera, mood). No permanent-identity tags.",
  "",
  "Editing rules:",
  "- When the user narrates the character, split the content into the correct fields. Appearance/identity goes to appearanceDescription+appearancePrompt; clothing to a new or existing outfit; place to background; action/scene to a new situation.",
  ...
  "- Keep identity, outfit, background, and situation cleanly separated so they can be recombined later. Never duplicate appearance tags into outfit/background/situation prompts.",
  "- For arrays (outfits, situations), return the FULL updated array in characterPatch, preserving existing items' id values and appending or editing as requested. When adding a new item, you may omit id (the client assigns one).",
