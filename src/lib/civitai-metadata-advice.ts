...
export function buildCivitaiMetadataAdvice({
  ...
  const fields: CivitaiMetadataField[] = [
    ...
    field("backend", "Generation backend", version ? "confirmed" : "inferred", version || params.backend),
  ...
  const recommendations: CivitaiGenerationRecommendation[] = [
    {
      ...
      description: hasAnyHires
        ? "Uses the published Hires recipe without changing it."
        : "Uses a conservative first-pass size without inventing a denoising Hires pass.",
      caution: !hasAnyHires && largeFinalImage
        ? "The published final size is larger; upscale the clean result separately if needed."
        : undefined,
      params: hasAnyHires
        ? { ...params }
        : { ...stableSamplerParams, backend: isIllustrious ? "forge" : params.backend, width: baseWidth, height: baseHeight, hires_upscale: 1, hires_steps: 0, upscale_model_name: "" },
    },
    {
      id: "literal-metadata",
    ...
    },
    {
      ...
      params: { ...stableSamplerParams, backend: isIllustrious ? "forge" : params.backend, width: baseWidth, height: baseHeight, hires_upscale: 1, hires_steps: 0, upscale_model_name: "" },
    },
    {
      id: "quality-priority",
      ...
      params: { ...stableSamplerParams, backend: isIllustrious ? "forge" : params.backend, width: baseWidth, height: baseHeight, sampler_name: isIllustrious ? "dpmpp_2m" : params.sampler_name, scheduler: isIllustrious ? "karras" : params.scheduler, num_inference_steps: Math.max(30, params.num_inference_steps ?? 0), hires_upscale: 1, hires_steps: 0, upscale_model_name: "" },
    },
  ];
