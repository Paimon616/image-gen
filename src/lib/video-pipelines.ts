...
const dasiwaMinimaxH3I2vaControls: VideoPipelineControl[] = [
  ...
  },
  {
    ...
    step: 0.05,
    group: "advanced",
    help: "MiniMaxH3Cache(node 1512:2722/2723)가 재사용을 시작하는 디노이징 진행률입니다. 원본 0.15. 높이면(예: 0.30) 구조가 잡히는 초반 step들이 캐시를 타지 않아 손가락 같은 미세 구조가 덜 깨집니다. Cache End %보다 크면 캐시가 사실상 꺼집니다.",
    patches: [
      { nodeId: "1512:2722", input: "start_percent" },
      { nodeId: "1512:2723", input: "start_percent" },
    ],
  },
  {
    key: "cache_end_percent",
    label: "Cache End %",
    type: "number",
    defaultValue: 0.9,
    min: 0,
    ...
    step: 0.05,
    group: "advanced",
    help: "MiniMaxH3Cache(node 1512:2722/2723)가 재사용을 멈추는 디노이징 진행률입니다. 원본 0.9. 낮추면(예: 0.8) 디테일이 정리되는 후반 step들이 항상 실제 계산되어 손·얼굴 품질이 좋아지는 대신 느려집니다.",
    patches: [
      { nodeId: "1512:2722", input: "end_percent" },
      { nodeId: "1512:2723", input: "end_percent" },
    ],
  },
  {
    key: "cache_max_steps",
    label: "Cache Max Steps",
    type: "number",
    defaultValue: 2,
    min: 1,
    max: 4,
    step: 1,
    group: "advanced",
    help: "MiniMaxH3Cache(node 1512:2722/2723)가 연속으로 재사용할 수 있는 최대 step 수입니다. 원본 2. 1로 낮추면 연속 재사용이 없어져 디테일 손실이 줄고, 높이면 더 빨라지는 대신 뭉개짐이 심해집니다.",
    patches: [
      { nodeId: "1512:2722", input: "max_steps" },
      { nodeId: "1512:2723", input: "max_steps" },
    ],
  },
  {
    key: "shift_video",
